...
class soundOut:
	def __init__(self, lower_bound=9, higher_bound=12, max_depth=8, random_state=None):
	...
	def fit(self, X):
		...
		for i in np.arange(self.lower_bound, self.higher_bound):
			...
			depths = np.mean(clf._compute_actual_depth(sample), axis=0)

			bins = np.arange(int(depths.min()), int(depths.max() + 2))
			y, x = np.histogram(depths, bins=bins)

			x_arr.append([i])
			y_arr.append(x[np.argmax(y)])
